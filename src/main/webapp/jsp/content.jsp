...
<div style="max-width: 1000px; margin: inherit;">
	...
	%>
		<h1>
			Total Results for <b>"<%=searchQuery %>"</b> :
			<%=(int) request.getAttribute("totalPages")%></h1>
...
<nav aria-label="Page navigation example">
	<ul class="pagination">
		...
		int totalResults = (int) request.getAttribute("totalPages");
		...
		<li class="page-item"><a class="page-link"
			href="<%=request.getContextPath()%>/SearchQuery?searchQuery=<%=searchQuery%>&pageNumber=<%=currentPage - 1%>">Previous</a>
		</li>
		<%
		}
		...
		<li class="page-item <%=activeClass%>"><a class="page-link"
			href="<%=request.getContextPath()%>/SearchQuery?searchQuery=<%=searchQuery%>&pageNumber=<%=i%>"><%=i + 1%></a>
		</li>
		<%
		}
		// Next button
		...
		%>
		<li class="page-item"><a class="page-link"
			href="<%=request.getContextPath()%>/SearchQuery?searchQuery=<%=searchQuery%>&pageNumber=<%=currentPage + 1%>">Next</a>
		</li>
		<%
		}
		%>
